                ...
                if os.path.isdir(item_path):
                    ...
                    try:
                        with open(item_path, 'r', encoding='utf-8') as file:
                            ...
                            f.write(f"{indent}{prefix}{'    ' if is_last else '│   '}Content:\n")
                            for line in content.splitlines():
                                f.write(f"{indent}{prefix}{'    ' if is_last else '│   '}    {line}\n")
                    except (UnicodeDecodeError, PermissionError, IOError):
                        f.write(f"{indent}{prefix}{'    ' if is_last else '│   '}Content: [Unable to read file]\n")
    except (PermissionError, OSError):
        with open(output_file, 'a', encoding='utf-8') as f:
            f.write(f"{indent}{prefix}└── [Permission denied or error accessing {path}]\n")
